...
public class ConditionPropertiesComposite extends AbstractBpmn2PropertiesComposite {
	...
	public void createBindings() {
		...
		if (be instanceof SequenceFlow) {
			...
			if (sf.getConditionExpression() instanceof FormalExpression) {
				FormalExpression fEx = (FormalExpression) sf.getConditionExpression();
				bindSequenceFlow(fEx);
			}
		}
		
	}
	
	private void bindSequenceFlow ( FormalExpression fEx) {
		ItemProviderAdapter itemProviderAdapter = (ItemProviderAdapter) new Bpmn2ItemProviderAdapterFactory().adapt(fEx,
				ItemProviderAdapter.class);

		EList<EAttribute> eAllAttributes = fEx.eClass().getEAllAttributes();
		Bpmn2Preferences preferences = Bpmn2Preferences.getPreferences(project);

		for (EAttribute a : eAllAttributes) {

			if (preferences.isEnabled(fEx.eClass(), a)) {
				IItemPropertyDescriptor propertyDescriptor = itemProviderAdapter.getPropertyDescriptor(fEx, a);

				if (String.class.equals(a.getEType().getInstanceClass())) {
					bind(a, createTextInput(propertyDescriptor.getDisplayName(fEx), propertyDescriptor.isMultiLine(fEx)));
				} else if (boolean.class.equals(a.getEType().getInstanceClass())) {
					bindBoolean(a, createBooleanInput(propertyDescriptor.getDisplayName(fEx)));
				} else if (int.class.equals(a.getEType().getInstanceClass())) {
					bindInt(a, createIntInput(propertyDescriptor.getDisplayName(fEx)));
				} else if (propertyDescriptor != null) {
					propertyDescriptor.getChoiceOfValues(fEx);
					createLabel(propertyDescriptor.getDisplayName(fEx));
					createSingleItemEditor(a, fEx.eGet(a), propertyDescriptor.getChoiceOfValues(fEx));
				}
			}
		}

		EList<EReference> eAllContainments = fEx.eClass().getEAllContainments();
		for (EReference e : fEx.eClass().getEAllReferences()) {
			if (preferences.isEnabled(fEx.eClass(), e) && !eAllContainments.contains(e)) {
				IItemPropertyDescriptor propertyDescriptor = itemProviderAdapter.getPropertyDescriptor(fEx, e);
				bindReference(e, propertyDescriptor.getDisplayName(e));
			}
		}

		if (fEx instanceof Participant) {
